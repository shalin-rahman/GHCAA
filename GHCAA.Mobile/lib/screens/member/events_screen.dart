...
class _EventsScreenState extends ConsumerState<EventsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    Future<void> processPayment(
        double amount, PaymentGateway gateway, String eventTitle) async {
      final gatewayService = ref.read(gatewayServiceProvider);
      final res =
          await gatewayService.initiate(amount, gateway, 'EVT-REG-$eventTitle');
    ...
    Future<void> registerFreeEvent(int eventId, String eventTitle) async {
      ...
      // member).
      final success =
          await ref.read(eventsServiceProvider).registerForEvent(eventId);
            ...
            children: [
              Text('SELECT PAYMENT METHOD',
                  ...
                  style: Theme.of(context)
                      .textTheme
                      .labelLarge
                      ?.copyWith(letterSpacing: 2, color: AppTheme.royalGold)),
              const SizedBox(height: AppTheme.spaceL),
              _paymentOption(context, 'SSLCommerz', 'Secure Pay with Cards/MFS',
                  Icons.account_balance_wallet_rounded, () async {
                Navigator.pop(context);
                await processPayment(
                    amount, PaymentGateway.sslCommerz, eventTitle);
              }),
              const SizedBox(height: AppTheme.spaceM),
              _paymentOption(context, 'DGePay', 'Debit/Credit Cards & Wallets',
                  Icons.credit_card_rounded, () async {
                Navigator.pop(context);
                        ...
                        child: filtered.isEmpty
                            ...
                            : ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  final requiresReg =
                                      ev['requiresRegistration'] != false;
                                  final fullImgUrl = AppConfig.resolveImageUrl(
                                      ev['coverImageUrl'] ?? ev['imageUrl']);
  ...
  bool _isRegistrationOpen(Map<String, dynamic> ev) {
    ...
    if (ev['registrationStartDate'] != null &&
        AppUtils.parseDate(ev['registrationStartDate'].toString())
                ?.isAfter(now) ==
            true) {
      return false;
    }
    if (ev['registrationEndDate'] != null &&
        AppUtils.parseDate(ev['registrationEndDate'].toString())
                ?.isBefore(now) ==
            true) {
      return false;
    }
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: AppTheme.spaceM),
                  _datePickerRow(context, 'Start Date', startDate,
                      (picked) => setDialogState(() => startDate = picked)),
                  const SizedBox(height: AppTheme.spaceS),
                  _datePickerRow(context, 'End Date', endDate,
                      (picked) => setDialogState(() => endDate = picked)),

                  const SizedBox(height: AppTheme.spaceL),
                  _sectionHeader(context, 'REGISTRATION DETAILS'),
                  const SizedBox(height: AppTheme.spaceM),
                  _datePickerRow(context, 'Opens', regStart,
                      (picked) => setDialogState(() => regStart = picked),
                      nullable: true),
                  const SizedBox(height: AppTheme.spaceS),
                  _datePickerRow(context, 'Closes', regEnd,
                      (picked) => setDialogState(() => regEnd = picked),
                      nullable: true),
                    ...
                    Padding(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                            labelText: 'Registration Fee (${currency.code})',
                            prefixIcon:
                    ...
                    children: [
                      ...
                      Transform.scale(
                        ...
                        child: Switch(
                          ...
                          onChanged: (v) =>
                              setDialogState(() => allowNonMembers = v),
                          activeThumbColor: AppTheme.royalGold,
                        ),
                      ),
                    ],
                    ...
                    children: [
                      ...
                      Transform.scale(
                        ...
                        child: Switch(
                          value: notifyMembers,
                          onChanged: (v) =>
                              setDialogState(() => notifyMembers = v),
                          activeThumbColor: AppTheme.royalGold,
                        ),
                      ),
                    ],
            ...
            ElevatedButton(
              onPressed: saving
                  ...
                  : () async {
                      ...
                      try {
                        final fee =
                            isFree ? null : (double.tryParse(feeCtrl.text));
                        final payload = {
                          ...
                          if (regStart != null)
                            'registrationStartDate': AppUtils.toWire(regStart!),
                          if (regEnd != null)
                            'registrationEndDate': AppUtils.toWire(regEnd!),
                        };
  ...
  Widget _sectionHeader(BuildContext context, String title) {
    return Text(title,
        style: Theme.of(context).textTheme.labelSmall?.copyWith(
            color: AppTheme.royalGold, letterSpacing: 1.5, fontSize: 8));
  }

  Widget _paymentOption(BuildContext context, String title, String subtitle,
