...
class _PermissionsMatrixScreenState
  ...
  Widget build(BuildContext context) {
    ...
    return AppScaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('NEW ADMIN', style: TextStyle(color: Colors.black, fontWeight: FontWeight.w900, fontSize: 11, letterSpacing: 1)),
      ),
      child: Column(
        children: [
          TabBar(
            ...
            tabs: const [
              Tab(text: 'SYSTEM USERS', icon: Icon(Icons.manage_accounts_outlined, size: 20)),
              Tab(text: 'ROLE REGISTRY', icon: Icon(Icons.shield_outlined, size: 20)),
            ],
          ),
          Expanded(
            child: TabBarView(
  ...
  Widget _buildUsersTab(AsyncValue<List<dynamic>> usersAsync) {
    return usersAsync.when(
      data: (users) => users.isEmpty
          ? const Center(child: Text('No system users found.', style: TextStyle(color: Colors.white38)))
          : ListView.builder(
              ...
              children: [
                CircleAvatar(
                  ...
                  child: Text(
                    ...
                    style: const TextStyle(color: AppTheme.royalGold, fontWeight: FontWeight.w900, fontSize: 16),
                  ),
                ),
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(user['username'] ?? 'Unknown', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14)),
                      Text(user['fullName'] ?? 'System Account', style: const TextStyle(color: Colors.white54, fontSize: 11)),
                    ],
                  ),
                ),
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: isActive ? Colors.greenAccent.withValues(alpha: 0.1) : Colors.redAccent.withValues(alpha: 0.1),
                    borderRadius: BorderRadius.circular(8),
                    border: Border.all(color: isActive ? Colors.greenAccent.withValues(alpha: 0.3) : Colors.redAccent.withValues(alpha: 0.3)),
                  ),
                  child: Text(isActive ? 'ACTIVE' : 'DISABLED', style: TextStyle(color: isActive ? Colors.greenAccent : Colors.redAccent, fontSize: 8, fontWeight: FontWeight.bold, letterSpacing: 1)),
                ),
              ],
            ),
            if (roles.isNotEmpty) ...[
              ...
              Row(
                children: [
                  const Icon(Icons.shield_moon_outlined, size: 14, color: Colors.white38),
                  const SizedBox(width: 6),
                  Expanded(
                    child: Wrap(
                      spacing: 6,
                      runSpacing: 4,
                      children: roles.map((role) => _buildRoleChip(role, userId)).toList(),
                    ),
                  ),
                  IconButton(
                    icon: const Icon(Icons.add_circle_outline, color: AppTheme.royalGold, size: 20),
              ...
              GestureDetector(
                ...
                child: const Row(
                  children: [
                    Icon(Icons.add_circle_outline, size: 14, color: Colors.white38),
                    SizedBox(width: 6),
                    Text('No roles assigned — tap to assign', style: TextStyle(color: Colors.white38, fontSize: 11)),
                  ],
                ),
              ),
            ],
    ...
    return rolesAsync.when(
      data: (roles) => Column(
        children: [
          ...
          Expanded(
            child: roles.isEmpty
                ? const Center(child: Text('No custom roles defined.', style: TextStyle(color: Colors.white38)))
                : ListView.builder(
                    padding: const EdgeInsets.symmetric(horizontal: 16),
                    itemCount: roles.length,
                    itemBuilder: (context, i) {
                      final role = roles[i];
                      final name = role is Map ? role['name'] ?? role.toString() : role.toString();
  ...
  Widget _buildRoleRow(String roleName) {
    final isSystem = roleName == 'SuperAdmin' || roleName == 'Admin' || roleName == 'Member';
    return Padding(
        ...
        child: Row(
          children: [
            Icon(isSystem ? Icons.lock_outlined : Icons.shield_outlined,
                color: isSystem ? AppTheme.royalGold : Colors.white54, size: 20),
            const SizedBox(width: 14),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  ...
                  Text(isSystem ? 'Built-in System Role' : 'Custom Institutional Role',
    ...
    final result = await showDialog<bool>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setState) => AlertDialog(
          ...
          title: const Text('CREATE SYSTEM ADMIN', style: TextStyle(color: AppTheme.royalGold, fontSize: 13, fontWeight: FontWeight.bold)),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              TextField(controller: userCtrl, style: const TextStyle(color: Colors.white), decoration: const InputDecoration(labelText: 'Username', prefixIcon: Icon(Icons.person_outline))),
              const SizedBox(height: 12),
              TextField(controller: passCtrl, obscureText: true, style: const TextStyle(color: Colors.white), decoration: const InputDecoration(labelText: 'Password', prefixIcon: Icon(Icons.lock_outline))),
              const SizedBox(height: 12),
              DropdownButtonFormField<String>(
                ...
                items: const [
                  ...
                  DropdownMenuItem(value: 'SuperAdmin', child: Text('Super Admin', style: TextStyle(color: Colors.white))),
                ],
  ...
  Future<void> _showAssignRoleDialog(BuildContext context, int userId) async {
    ...
    final result = await showDialog<bool>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setState) => AlertDialog(
          backgroundColor: AppTheme.midnightSurface,
          title: const Text('ASSIGN ROLE', style: TextStyle(color: AppTheme.royalGold, fontSize: 13, fontWeight: FontWeight.bold)),
          content: DropdownButtonFormField<String>(
            ...
            items: allRoles.map((r) {
              ...
              return DropdownMenuItem<String>(value: name, child: Text(name, style: const TextStyle(color: Colors.white)));
            }).toList(),
