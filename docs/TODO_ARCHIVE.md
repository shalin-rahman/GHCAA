...
which already resolves Tier 3 files to `profiles/<name>/demo-data/` and Tier 1/2 files to
...
**Update 2026-09-16:** the mechanism is fixed — `HasData` no longer declares any Tier 3 entity (see
82.31 below for the migration side), and `InstitutionDataSeeder` seeds those tables at boot from
`profiles/<profile>/demo-data/*.json` or, if set, an operator-supplied `AppSettings:RealDataPath`,
only when the table is empty. What is still open: the committed copies at
`profiles/ghc/demo-data/*.json` still hold GHC's real 631 alumni records today. Externalising them
(copy to a location outside the git tree, verify the copy, then replace the committed files with
empty arrays and point GHC's deployment at `AppSettings:RealDataPath`) was proposed and the project
owner's answer was "ignore this ... wont do" — parked by decision, not forgotten. Nothing was moved
or deleted; the real files are untouched in the git tree.

62.32 [DONE 2026-09-05] All 15 Tier 3 files (including `users.json`/`user_roles.json`) moved to
...
1. `ApplicationDbContext.LoadSeed`'s design-time detection
   ...
   silently routed every Tier 3 file through the profile-pack path even when a test factory asked
...
what stays shared (Tier 1), the environment variables that are theirs to set (including the new
...
**Update 2026-09-16:** the committed-migration half of this is done. `ApplicationDbContext` no longer
declares Tier 3 entities via `HasData`; the migration that would have scaffolded `DeleteData` for
them (`RemoveInstitutionSeedData`) is a hand-edited no-op so it doesn't touch GHC's live rows, and
`InstitutionDataSeeder` now owns seeding those tables at runtime instead. A clean clone still needs
the *already-committed* eight migrations rewritten out of git history to actually stop carrying the
631 hashes — that rewrite is the separately-confirmed, force-push step in the governing plan and has
not been run.

The forced password reset is done, gated, and off by default: `Constants.ConfigKeys.ForcePasswordResetOnBoot`
(`AppSettings:ForcePasswordResetOnBoot`, absent = false) controls a boot step in
`DatabaseBootstrapperExtensions` that flags every account without `MustChangePassword` set, and does
nothing when the key is unset or false. Turning it on for existing accounts is the project owner's
call, to be made once the history rewrite actually happens — flipping it before then would reset
passwords for hashes still sitting in git.

**Still not started, and deliberately not started here:** externalising `profiles/ghc/demo-data/*.json`'s
real content out of the git tree (see 62.31's 2026-09-16 update) and the git history rewrite itself.
Both are P0 data-protection work the project owner has put on hold. Recorded so the gap is not
rediscovered a fourth time; no file touched.
...
83.3 [DONE 2026-09-16] **Priority: P2 | Depends on: 83.2.** The 83.2 extraction of
`GenerateMembershipNumber` used `.FirstOrDefault()` instead of the original
`.FirstOrDefaultAsync(cancellationToken)` — a regression that blocked a thread-pool thread on the
lookup query and silently dropped the `CancellationToken`, inconsistent with every other DB call
in the same class. Found reviewing commit `d08b8350` against the standing PR checklist.
Fix: `GenerateMembershipNumber` and its caller `BuildMemberFromDto` are now
`GenerateMembershipNumberAsync`/`BuildMemberFromDtoAsync`, both awaited from `RegisterAsync` with
the request's `CancellationToken` threaded through to `FirstOrDefaultAsync`.
**Acceptance:** `GHCAA.Infrastructure` builds clean (0 warnings, 0 errors); all 37
`MemberServiceTests` pass unchanged.

62.54 [DONE 2026-09-16] **Priority: P4 | Depends on: none.** Found by 62.52, revised 2026-09-08 after checking
...
frontend fallback matching the backend's shared enum is correct as-is, not a coincidence to fix.

**Resolved 2026-09-16:** removed. Checking the Angular side turned up one more consumer the earlier
pass missed: the admin org-config editor (`org-config.ts`/`.html`) had a comma-separated
"Membership Types" field that read and wrote `workflow.membershipTypes`, and
`org-config.model.ts`/`org-config-fallback.generated.ts` carried the same shape — none of it fed
anything downstream, it just round-tripped a value nothing read. Removed `MembershipTypes` from
`OrgConfigDto`, the `OrgConfigService` default builder, both `profiles/*/org-config.json` files, the
golden test snapshot, the admin editor field, `org-config.model.ts`, and the fallback generator
script, then regenerated `org-config-fallback.generated.ts`. `dotnet build` on the full solution,
`tsc --noEmit`, and the 21 OrgConfig tests all pass clean.

62.24 [DONE 2026-09-16] **Priority: P3 | Depends on: 62.15.** Verify the gold palette (`--accent-color: #c5a059`,
`--gold-gradient` in `styles.scss` L24-130) is only a seeded default and not a hard dependency, given
`admin-themes` makes themes admin-configurable.
**Resolution 2026-09-16:** the hard dependency found in the 2026-09-07 pass (`--gold-gradient` and its
siblings never derived from `--accent-color`) is now fixed. Added `GHCAA.Web/src/app/core/utils/color-math.ts`
(HSL conversion, WCAG contrast ratio, an HSL-lightness contrast-clamp loop) and extended
`OrgConfigService`'s constructor effect to derive `--accent-color-rgb`/`--accent-rgb`, `--accent-gold-bright`,
`--accent-gold-dark`, `--accent-text`, `--shadow-gold`, `--gold-gradient`, `--tier-founding`, `--glass-border`
and `--select-chevron-gold` from the profile's `--accent-color` on every config/theme change, instead of
leaving them as static literals. `--accent-gold-dark`/`--accent-text` (4.5:1) and `--tier-founding` (4.9:1)
are darkened until they clear the same AA floors the old hand-tuned literals were built to hit, rather than
just tinting them, so a custom accent color can't ship an inaccessible token. `--glass-border` in dark theme
is left as `styles.scss`'s own independent translucent-card value (not accent-derived there) via
`removeProperty`. Fixed the hardcoded `%23c5a059` SVG stroke in `admin-payment-config.scss` by moving the
whole `url(...)` value into a computed `--select-chevron-gold` custom property, since a data-URI's own
attributes can't reference `var()` directly. Static `styles.scss` literals are left as-is: they're the
correct pre-JS-load fallback and are overridden by the inline `setProperty` calls regardless of their
literal value, so rewriting them as self-referencing `var(x, fallback)` would add no behavior. Verified with
`GHCAA.Web/src/app/core/utils/color-math.spec.ts` (new) and an expanded
`GHCAA.Web/src/app/core/services/org-config.service.spec.ts`; `npx vitest run` (85 files, 489 tests) and
`npx tsc --noEmit -p tsconfig.app.json` both clean.

62.22 [DONE 2026-09-16] Added the general governing-document registry: `Documents` (list of
`DocumentEntryDto { Label, Url, Version, Group }`) on `OrgConfigDto`, populated in both profile
packs (`profiles/ghc/org-config.json` carries the constitution PDF as its one entry;
`profiles/default/org-config.json` is empty), mirrored in `OrgConfigService.BuildGhcaaDefaults()`,
and reflected in the golden snapshot and `GHCAA.Tests/OrgConfig` suite (21/21 passing). Angular
side: `DocumentEntry` on `org-config.model.ts`, `generate-org-config-fallback.mjs` and
`org-config-fallback.generated.ts` updated and type-checked clean. `Branding.ConstitutionPdfUrl`
(added under this item's earlier partial pass) is kept as-is rather than migrated into
`Documents` — low churn, and existing consumers (`constitution.ts`'s `pdfUrl` fallback) still read
it directly. A dedicated Angular list/table UI to render `Documents` is not built yet; nothing
in the app renders the registry today, so that's left for whichever page first needs it.

**Follow-up 2026-09-16, same day:** both 62.54 and this item's Angular pass had missed
`GHCAA.Mobile`. `org_config.dart` (documented as matching `OrgConfigDto.cs` schema exactly)
still declared the removed `membershipTypes` field on `OrgWorkflow`, and never picked up the
new `documents` field at all. Fixed: dropped `membershipTypes` from `OrgWorkflow`, added a
`DocumentEntry` class (`label`/`url`/`version`/`group`) and a `documents` list (default `[]`,
so old cached JSON without the field still parses) on the root `OrgConfig`, and updated
`offlineDefaults` to match. `flutter analyze` clean, `flutter test test/org_config_service_test.dart`
4/4 passing (added one case for the populated registry, one for the field-absent default).
Also added the `documents` passthrough case to `org-config.service.spec.ts`, which had gone
in without one; `npx vitest run` on that spec now 19/19. Lesson for future shared-DTO changes:
audit backend + Angular + Flutter mobile together, since the plan that scoped 62.54/62.22 named
only the Angular side.

79.5 [DONE 2026-09-17] **Priority: P3.** Swept user-facing strings against SR-1 in both clients.
Web: 5 strings reworded — the global HTTP interceptor's default error ("An unexpected error
occurred" -> "Something went wrong"), the member-approval confirm dialog ("This will officially
induct the member and dispatch credentials" -> "This will approve them and send their login
details"), the Excel import error, the assistant's fallback reply (dropped the "neural core"
line), and the payment-status success/failure copy (dropped "financial routing"/"payment network
payload"). Mobile: 10 strings reworded — the four generic "An unexpected error occurred[.]" auth
fallbacks and the two login-screen ones collapsed to "Something went wrong[.]", the assistant's
"I am currently unable to process your request" fallback, the family-link removal dialog ("This
will decouple your digital identity from this member" -> "This will unlink your account from
this member"), the custom-role delete dialog's "lose associated permissions" phrasing, and the
reject-reason dialog's "Please provide a reason for rejection" opener. Grepped both test suites
for each string before touching it; none were asserted on, so no test assertions needed updating.
Left the "Are you sure you want to delete X? This action cannot be undone." confirm-dialog
pattern alone across both clients — already plain and consistent, not a filler/AI-sounding
construction. `npx tsc --noEmit` clean; `npx vitest run` 85 files / 490 tests passing. `flutter
test` has 9 pre-existing failures from an unrelated, already-modified `support_service.dart` /
...
replaces) to `OrgConfigDto`, exposed as a select on the Workflow tab of the admin org-config page
(`org-config.html`/`org-config.ts`, `notificationChannels` array). The actual dispatch chokepoint
turned out to be `AdminNotificationService.NotifyPendingApprovalAsync`, not `NotificationService` or
`MessagingController` (the latter is the chat controller, unrelated) — it's the one place in the
codebase that already fans a member notification out across more than one channel. It now reads
`IOrgConfigService.GetConfigAsync().Workflow.NotificationChannel` and skips `IEmailService.SendEmailAsync`
when the channel is "Sms" and skips `ISmsService.SendAlertSmsAsync` when the channel is "Email"; the
in-app notification row always fires regardless, since that's a separate channel from the SMS/Email
toggle. This also gives `GreenwebSmsService` its first real caller — it existed fully implemented but
unused before this change. Left `CommunicationService`'s direct email sends (OTP, password reset,
campaigns) ungated: those are transactional/deliberate sends an admin explicitly triggers or that
auth depends on, not the "notifications" this item is about, and gating OTP delivery behind this
toggle with no SMS OTP path wired would break login/registration. Updated the golden snapshot
(`org-config-ghc.golden.json`), both profile packs (`profiles/default`, `profiles/ghc`), and
regenerated `org-config-fallback.generated.ts` via `npm run gen:org-config` rather than hand-editing
it. Tests: `AdminNotificationServiceTests` — 3 new cases (`NotifyPendingApprovalAsync_ChannelEmail_SkipsSms`,
`NotifyPendingApprovalAsync_ChannelSms_SkipsEmail`, `NotifyPendingApprovalAsync_ChannelBoth_SendsEmailAndSms`)
plus the 3 pre-existing ones, all 6 passing; full `GHCAA.Tests` OrgConfig + AdminNotificationService
filter run: 27 passed, 0 failed. `dotnet build` clean, `npx tsc --noEmit` clean.
