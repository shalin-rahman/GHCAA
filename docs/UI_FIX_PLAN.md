...
> **Note (2026-08-02, Work Package 33 review):** 2.9's empty-state fix and 2.8's identity-line fix both shipped (30.22/30.23), but the Work Package 33 holistic pass (`docs/TODO.md` 33.1/33.4) found `.empty-state.compact` only reached the edit-form's *duplicate* empty-state blocks in `profile.html`, not the read-only history sections above them (EC/Academic/Professional) — follow-up logged as `docs/TODO.md` 33.8. Work Package 33 also found the member-profile vs. admin-member-detail section order still diverges for the same entity (never covered by this plan) — see `docs/TODO.md` 33.2/33.9/33.12 — and that several form labels/headings are jargon-heavy (33.7).
