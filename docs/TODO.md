...
82.74 [IN PROGRESS 2026-09-11] **Priority: P2 | Depends on: 82.73.** Enforce reusable component consumption for all repeated controls and component surfaces across Angular Web. Existing shared headers, search, pagination, dialogs, modal headers, tables, and loading panels are being reused; remaining work is the control-by-control audit and exception record.
...
82.75 [IN PROGRESS 2026-09-11] **Priority: P1 | Depends on: 82.74.** Audit admin tables and grids for search, pagination, sorting, loading, empty states, responsive overflow, and row actions. The first inventory confirms most operational tables already use `SearchBarComponent`; fixed/small configuration tables such as fee and payment settings remain intentional no-search candidates, while polls, event operations, and any remaining data-heavy grids still need a decision. Add the shared search control only where the dataset and task require it; record why a small or fixed dataset does not need search. Reuse `table-pagination.util.ts` and the centralized table classes. **Acceptance:** every admin table has a documented search decision, required searches use the shared themed control, and focused Angular tests cover filtering and reset behavior.
