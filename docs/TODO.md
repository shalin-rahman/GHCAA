...
## AREA 33: OVERALL DESIGN/LAYOUT & UX REVIEW (raised by user 2026-08-02)

33.1 [TODO] REVIEW: Overall design and layout consistency across web (admin + portal + public) and mobile — after all the piecemeal Area 30 fixes, do a holistic pass rather than page-by-page: spacing/density rhythm, card/section visual hierarchy, color/token usage consistency, whether shared components (`.glass-card`, `.form-group`, `.btn*`, `app-icon`, `app-logo-spinner`, `app-user-menu`) are applied uniformly or if any page still diverges. Produce a punch list of concrete inconsistencies (page + description), don't just give a general impression.
33.2 [TODO] REVIEW: Sequence/order of content on each major page and in navigation — does the order of sections match user priority/expectation (e.g. most-used info first, logical reading order top-to-bottom, related items grouped)? Check landing page section order, dashboard widget order, profile section order, nav item order within each `section` (Overview/Community/Directory/Career/My Account), and admin sidebar grouping. Flag any section that's ordered awkwardly or buried where a user wouldn't expect it.
33.3 [TODO] REVIEW: User-friendliness/usability pass — clarity of labels and instructions, discoverability of key actions (building on 30.36's admin-panel-discoverability fix — check for other "hidden feature" cases), error-message quality, empty-state guidance, form validation feedback, mobile touch-target sizing, and overall first-time-user comprehension. Where feasible, actually walk through the golden path as a new member and as an admin (per the `verify` skill) rather than reasoning about it abstractly.
33.4 [TODO] Triage findings from 33.1-33.3 into: (a) quick centralized token/shared-component fixes, (b) per-page reorder/restructure changes, (c) larger UX redesign items needing a separate plan doc before implementation. Do NOT implement ad-hoc during the review — this item is investigate-and-triage only; log concrete follow-up TODOs (new Area 33.5+ or fold into existing Area 30 items) once triage is done, then implement in a later pass following the same fix-centrally rule as Area 30.
