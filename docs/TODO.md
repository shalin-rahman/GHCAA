...
82.84 [BLOCKED 2026-09-11] **Priority: P1 | Depends on: tooling availability.** **Canonical detail for WP60.4.** Complete mobile authenticated integration verification currently blocked by Windows native tooling: `flutter_secure_storage_windows` cannot find `atlstr.h`, and the Firebase SDK archive reports ZIP decompression failure. Use the Windows desktop toolchain after the Visual Studio ATL/C++ components and SDK download path are repaired, or an Android emulator; do not treat Chrome `integration_test` as a valid substitute while Flutter reports it unsupported. **Acceptance:** login, dashboard, logout, and at least one authenticated forum flow pass on a supported device, with the toolchain result recorded in `docs/BUSINESS_FINDINGS.md`.
...
82.93 [TODO] **Priority: P3 | Depends on: 82.92.** Refresh the mobile golden baselines as separate housekeeping work. Record the exact update command and resulting count; do not use golden refreshes to hide functional or theme regressions. **Acceptance:** the baseline refresh is independently reviewable and the non-golden suite remains green.
