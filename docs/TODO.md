...
12.5 [DONE 2026-09-17] **Priority: P3.** Mobile: On any unhandled error, show option to "Send Report to Administrator" with log attachment. Implemented as a second button on the ErrorWidget.builder fallback screen, next to the existing Sentry "DIAGNOSE & REPORT" button. It posts the exception message, a 5-line stack summary, and platform info through the same /contact pathway support_screen.dart already uses, so it lands in the same admin inbox as a member support message. No full log file is attached — 12.3's rotating log capture isn't built yet, so this sends only the immediate error context. VERIFIED 2026-09-17: `flutter test test/admin_report_button_test.dart` (2 tests, send + retry-on-failure states) and `flutter analyze` on the touched files, both clean.
