...
32.1 [TODO] INVESTIGATE: Member profile page is missing/showing incorrect values for fields that were previously populated and displayed correctly (user specifically flagged Educational Info and Professional Info sections). Compare, per member: (a) what's in `Seed/*.json` (or whichever seed source is authoritative), (b) what's actually in the live DB tables (per [[gotcha_seed_json_vs_live_db.md]] — editing Seed JSON alone does NOT update an already-created SQLite DB, so seed and live DB can and do diverge; check the live DB directly, not just the seed files), and (c) what the profile API response / `profile.html` actually renders. Identify whether this is a data-loss bug (values never made it into the DB), a mapping/DTO bug (values exist in DB but aren't returned/rendered), or a stale-seed bug (DB was seeded before a field was added/renamed and never re-seeded).
32.2 [TODO] Once root cause of 32.1 is confirmed, audit OTHER profile-adjacent areas for the same class of bug before assuming it's isolated to Education/Professional Info — user explicitly suspects other profile areas may have the same problem. Check: Member Directory cards, EC/Governance cards (also flagged missing PassingYear/Degree/Subject in 30.27 — likely the SAME root cause, cross-reference before duplicating the fix), Digital ID, and any admin member-detail view that reads the same Member entity.
32.3 [TODO] Fix the confirmed root cause(s) from 32.1/32.2 centrally (DTO/mapping fix in Application/Infrastructure layer if it's a mapping bug; DB update/re-seed script if it's a data bug) — do not patch the display layer if the underlying data/mapping is wrong. Add/extend a test that would have caught the regression. Verify `dotnet test` + relevant web test(s) stay green, then update `docs/TODO.md` and any other `docs/` file describing profile data flow (e.g. `docs/architecture_data_flow.md`, `docs/BUSINESS_FINDINGS.md`) that now needs correction.
...
33.5 [TODO] (a) Replace the remaining raw emoji action icons (`🗑️` `📷` `🖊️` `💾` `✕` `🚀`) in `member/profile/profile.html` and the admin member-detail modal in `admin/members/admin-members.html` with `<app-icon>`, completing the 30.3 icon-system rollout for these two forms specifically.
33.6 [TODO] (a) Replace `profile.html`'s bespoke `.remove-photo-btn` (28px) with the shared `.btn-icon.danger` (36px) class from 30.2 so photo/signature-removal buttons match the established icon-button taxonomy instead of a one-off size/style.
33.7 [TODO] (a) Simplify jargon-heavy labels/headings identified in 33.3(1) to plain language centrally in `profile.html` (and mirror in the admin member-detail modal where the same concept is labeled differently), so nav label / dashboard checklist label / on-page section heading all agree, extending the single-source-of-truth principle 30.11 already applied to page titles.
33.8 [TODO] (a) Apply `.empty-state.compact` (30.5) to the *top* read-only empty-state blocks in `profile.html` (EC History L96, Academic L130, Professional L175) — 30.22 only reached the edit-form's duplicate empty states (L386/L477), not these.
33.9 [TODO] (a) Reconcile `profile.scss`'s dead `.status-grid { grid-template-columns: repeat(3, 1fr) }` with the template's actual `md:grid-cols-4` utility class (33.1 finding 5) — pick one source of truth and delete the other to prevent a future edit from silently doing nothing.
33.10 [TODO] (b) Reorder `member/profile/profile.html` so the editable identity form comes immediately after the membership-status card, with the read-only EC/Academic/Professional history timelines moved below (or merged into) their corresponding edit sections, so a new member reaches an actionable field before three empty-state cards.
33.11 [TODO] (b) Reorder `public/landing.html` sections so `landing-ec-preview` (leadership/trust signal) moves earlier (e.g. directly after `landing-purpose`) and `landing-jobs` moves after `landing-membership`, so the join-CTA and leadership credibility surface before a job-board teaser.
33.12 [TODO] (c) Design (separate plan doc) a single shared "member data" sub-component/template consumed by both `member/profile/profile.html` and the admin member-detail modal in `admin-members.html`, so the two views of one Member entity stop drifting in section order and visual treatment (33.1 finding 1, 33.2 finding 2) whenever either is edited in isolation.
33.13 [TODO] (c) Live-browser Playwright/manual walkthrough (per the `verify` skill) of the profile page (member + admin) and the rest of the app's golden paths — still owed, no browser was available in the 33.1-33.3 review session.
