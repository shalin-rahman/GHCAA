    ...
    public partial class MemberService : IMemberService
    {
        ...
        public async Task<int> RegisterAsync(MemberRegistrationDto dto, UploadedFileDto? photo, UploadedFileDto? certificate, UploadedFileDto? paymentProof, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                await ProcessRegistrationPaymentAsync(member, dto, assignedType, paymentProof, cancellationToken);
                await SaveRegistrationUploadsAsync(member, photo, certificate, cancellationToken);
            ...
            // WP83: OTP send and the admin realtime alert are external calls (SMTP/websocket) that used
            // to run inside the Serializable-isolation transaction above. Holding the transaction open
            // across a slow external call widened the conflict window and could trip Postgres's
            // idle-in-transaction timeout, which surfaced as "This NpgsqlTransaction has completed; it
            // is no longer usable" at commit. The registration itself is already committed by this
            // point, so a failure here is logged, not thrown. The applicant still has a row, they just
            // may need the OTP resent.
            try
            {
            ...
            {
                _logger.LogError(ex, "Registration OTP send failed for MemberId {MemberId}", member.Id);
            }
            ...
            {
                _logger.LogError(ex, "Admin realtime alert failed for MemberId {MemberId}", member.Id);
            }

            return member.Id;
        }

        private async Task EnsureNoDuplicateMemberAsync(MemberRegistrationDto dto, CancellationToken cancellationToken)
        {
        ...
        private async Task<Enums.MembershipType> ResolveAssignedMembershipTypeAsync()
        {
            var orgConfig = await _orgConfigService.GetConfigAsync();
            return Enum.TryParse<Enums.MembershipType>(orgConfig?.Workflow?.DefaultMembershipType, true, out var defaultType)
                ? defaultType
        ...
        private async Task AddAcademicHistoryAsync(Member member, System.Collections.Generic.IList<AcademicRecordDto>? academicHistory, CancellationToken cancellationToken)
        {
            if (academicHistory == null || !academicHistory.Any())
            {
            ...
            EnforceInstitutionalAcademicRecord(academicHistory, institutionName);

            foreach (var a in academicHistory)
            {
        ...
        private void AddProfessionalHistory(Member member, System.Collections.Generic.IEnumerable<ProfessionalRecordDto>? professionalHistory)
        {
            if (professionalHistory == null)
                return;

            foreach (var p in professionalHistory)
            {
        ...
        private async Task ProcessRegistrationPaymentAsync(Member member, MemberRegistrationDto dto, Enums.MembershipType assignedType, UploadedFileDto? paymentProof, CancellationToken cancellationToken)
        {
        ...
        private async Task SaveRegistrationUploadsAsync(Member member, UploadedFileDto? photo, UploadedFileDto? certificate, CancellationToken cancellationToken)
        {
